    ...
    return {
        ...
        updatePageStates: function () {
            ...
            for (i = 0, len = this.$pages.length; i < len; ++i) {
                ...
                y0 = rect.top + scrollTop - offset.top;
                x0 = rect.left + scrollLeft - offset.left;
                page = pages[i] || {
                    index: i
                };
...
Crocodoc.addModule('lazy-loader', function (scope) {
    ...
    var util = scope.getService('util'),
        ...
        pages,
        numPages,
        afterscrollTriggerUnloadingTID,
        ...
        if (pageLoadQueue.length > 0) {
            ...
            api.loadPageIfNecessary(index, function loadPageCallback(error) {
                if (error) {
                    ...
                    // @TODO(clakenen): maybe don't keep trying to load this infinitely?
    ...
    return util.extend(api, {
        ...
        onmessage: function (name, data) {
            switch (name) {
                ...
                case 'afterscroll':
                    this.loadNecessaryPages();
                    clearTimeout(afterscrollTriggerUnloadingTID);
                    afterscrollTriggerUnloadingTID = setTimeout(function () {
                        api.unloadUnnecessaryPages(pageLoadRange);
                    }, 100);
                    break;
                case 'pagefocus':
                    this.updateLayoutState(data);
...
Crocodoc.addModule('page', function (scope) {
    ...
    var CSS_CLASS_PAGE_INNER = 'crocodoc-page-inner',
        CSS_CLASS_PAGE_LOADING = 'crocodoc-page-loading',
        CSS_CLASS_PAGE_TEXT = 'crocodoc-page-text',
        CSS_CLASS_PAGE_SVG = 'crocodoc-page-svg',
        CSS_CLASS_PAGE_LINKS = 'crocodoc-page-links',
        CSS_CLASS_PAGE_OVERLAY = 'crocodoc-page-overlay';
    ...
    return {
         ...
         * @param  {Function} callback Callback function to be executed after the SVG loads
         * @returns {void}
         */
        load: function (callback) {
            shouldLoad = true;
            ...
            pageText.load();
            pageSVG.load(svgLoadHandler);

            function svgLoadHandler(error) {
                if (!error) {
...
Crocodoc.addModule('viewer-api', function (scope) {
    ...
    return util.extend(api, {
        ...
        setLayout: function (mode) {
            ...
            // force it into presentation mode so the page is vertically
            // centered in the viewport
            // @TODO: only do this if the page is smaller than the viewport at normal zoom
